...
## Making Non-Semantic Elements (e.g., div) Accessible

- **Prefer semantic elements**: Use `<button>`, `<a>`, `<input>`, etc., whenever possible for interactive controls.
- **If you must use a `<div>` or `<span>` as an interactive element:**
  1. Add `tabIndex={0}` to make it focusable by keyboard.
  2. Add a suitable `role` (e.g., `role="button"`, `role="link"`) to communicate purpose to assistive tech.
  3. Add `aria-label` or `aria-labelledby` for accessible naming if the label is not visible.
  4. Implement keyboard event handlers for `Enter` and `Space` to trigger the same action as a mouse click.
  5. Ensure visible focus styles are present and not removed.
  6. Only use this pattern when semantic elements are not feasible.
...
- **Warning:** Overusing non-semantic elements for interactivity can harm accessibility. Always prefer native elements unless there is a strong technical reason.
